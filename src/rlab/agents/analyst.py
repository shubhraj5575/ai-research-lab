...
class DataAnalyst(Agent):
    ...
    def analyze(self, session_id: str, exp: Experiment) -> Analysis | None:
        ...
        if not runs:
            return None
        metric = exp.config.extra.get("primary_metric")  # optional override
        by_variant: dict[str, dict[int, RunResult]] = {}
        for r in runs:
            value = r.metrics.get(metric or "")
            if value is None:
                continue
            by_variant.setdefault(r.variant, {})[r.seed] = r
        if metric is None:
            # infer: first metric name common to all variants
            common = None
            for variant, seeded in by_variant.items():
                keys = set(next(iter(seeded.values())).metrics.keys())
                common = keys if common is None else (common & keys)
            if not common:
                return None
            metric = sorted(common)[0]
        values = {
            v: {seed: rr.metrics[metric] for seed, rr in seeded.items()}
            for v, seeded in by_variant.items()
        }
        if len(values) < 1:
            return None

        direction = "minimize"
        means = {v: sum(d.values()) / len(d) for v, d in values.items()}
        ranking = sorted(means.items(), key=lambda kv: kv[1])  # ascending
        baseline_label = exp.config.baseline if exp.config.baseline in values else ranking[-1][0]

        comparisons: list[Comparison] = []
        pvalues_family: list[float] = []
        raw_comparisons: list[tuple[str, float]] = []

        base_seeds = values[baseline_label]
        for variant in sorted(values):
            if variant == baseline_label:
                continue
            comp = self._compare(variant, values[variant], baseline_label,
